...
public class AddScheduleDoctorCommandTests
{
    ...
    public async Task Handle_WhenTimeStampRangeIsInvalid_ShouldReturnFailure()
    {
        ...
        _doctorRepository.ExistByIdAsync(invalidCommand.DoctorId, Arg.Any<CancellationToken>()).Returns(true);
        _doctorScheduleRepository.ScheduleIsFreeAsync(invalidCommand.Date, invalidCommand.Start, invalidCommand.End, Arg.Any<CancellationToken>()).Returns(true);
